...
- **Fix SW doble banner (25-Jun-2026)**: En App.tsx, el `useEffect` de detección de actualización SW tenía dos listeners (`reg.installing` + `updatefound`) que se disparaban ambos para el mismo SW en Chrome/Android, causando dos llamadas a `activateUpdate` y duplicación del banner. Fix: `useRef<Set<ServiceWorker>>` para deduplicar — aunque `activateUpdate` se invoque múltiples veces con la misma instancia de SW, solo procesa la primera.
